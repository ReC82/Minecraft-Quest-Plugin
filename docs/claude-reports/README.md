...
| 2026-09-06 | Issue #10 — audit TLS FTPS VeryGames (tests non destructifs, vrais identifiants) : FTP en clair **refusé** (530), **AUTH TLS explicite obligatoire** ; chaîne de certificat **incomplète** servie par ProFTPD → nouveau `scripts/verygames-fetch-ca.sh` (reconstruit les intermédiaires Let's Encrypt via AIA, vérifie contre le magasin système, écrit un PEM public) + variable `VERYGAMES_FTP_CA_EXTRA`, **jamais `--insecure`**. Défaut `VERYGAMES_FTP_TLS=require` (`auto` = alias). Login FTP = `<slot>.<sous-compte>` (`si-16041.awsplugin`, pas `awsplugin`). Diagnostics curl parlants ; `--check` liste le dossier distant. Aucun fichier transféré. | PARTIAL (blocage externe : mot de passe du compte FTP erroné dans verygames.env) | [2026-09-06_0840_verygames-tls-ftps-audit-issue-10.md](2026-09-06_0840_verygames-tls-ftps-audit-issue-10.md) |
