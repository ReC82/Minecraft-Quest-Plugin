...
| 2026-09-05 | Issue #27 — permissions granulaires par rôle / monde / action : nouveau paquet `permission` (`BuildPermissionService`, `BuildArea`, `RpgQuestPermissions`) + `config.PermissionsConfig` (`permissions.build-areas`). Construction par Hub/monde (`rpgquest.build.hub.<id>\|.*\|.wild\|.zone\|.world.<clé>\|.*`), `/rpgadmin` découpé en un nœud par sous-commande, claims séparés (`rpgquest.claim.bypass\|.admin`). **Aucune** permission de build ne contourne un claim ; tout est fille de `rpgquest.admin.world` (OP inchangé). Compatible LuckPerms non obligatoire. `plugin.yml` + `config.yml` enrichis, nouveau `docs/PERMISSIONS.md`. Tests : `BuildPermissionServiceTest`, `RpgAdminCommandPermissionsTest`, ajouts `Claim`/`Hub`/`ConfigValidator`. | DONE | [2026-09-05_2314_permissions-granulaires-issue-27.md](2026-09-05_2314_permissions-granulaires-issue-27.md) |
